...
mod windows {
    ...
    const CLASS_NAME: &HSTRING = w!("TAURI_DRAG_RESIZE_BORDERS");
    const WINDOW_NAME: &HSTRING = w!("TAURI_DRAG_RESIZE_WINDOW");
    ...
    unsafe extern "system" fn drag_resize_window_proc(
        ...
        match msg {
            WM_NCHITTEST => {
                ...
                let mut rect = RECT::default();
                if !GetWindowRect(child, &mut rect).as_bool() {
                    return DefWindowProcW(child, msg, wparam, lparam);
                }
    ...
    fn is_maximized(window: HWND) -> windows::core::Result<bool> {
        ...
        Ok(placement.showCmd == SW_MAXIMIZE)
    }
